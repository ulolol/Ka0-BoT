...
"""
    Ka0$BoT Reasoning Edition (o1-mini)
...
class OpenAIWebWrapper:
    def __init__(self, model_name: str = "o1-mini"):
    ...
    async def query_openai_async(self, prompt: str, context: str, chat_history: list = []) -> any:
        ...
        try:
            ...
            messages.append({"role": "assistant", "content": f"Previous chat history for Reference: \n {chat_history}"})
            #messages.extend(chat_history)
            if context:
                messages.append({"role": "assistant", "content": f"Web Search Context: \n{context}"})
...
class GradioAIChatbot:
    def __init__(self):
        self.wrapper = OpenAIWebWrapper(model_name="o1-mini")
        self.chat_history = []
        self.num_results = 5
